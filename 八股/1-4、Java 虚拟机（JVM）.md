...
本地方法被执行的时候，在**本地方法栈**也会创建一个`栈帧`，用于存放该本地方法的**局部变量表**、**操作数栈**、**动态链接**、**出口信息**。
...
|   JDK版本   | 方法区具体实现 |      方法区物理区域       |                    运行时常量池 物理区域                     |
| :---------: | :------------: | :-----------------------: | :----------------------------------------------------------: |
| <  **JDK6** |     永久代     |    永久代 （PermGen）     |                            永久代                            |
|  **JDK 7**  |     永久代     |        堆 + 永久代        | 字符串常量池、静态变量 ：堆<br />其他信息（类型信息等）：永久代 |
|  **JDK 8**  |     元空间     | 堆 + 元空间（Meta-space） |              字符串常量池：堆；其他信息：元空间              |
